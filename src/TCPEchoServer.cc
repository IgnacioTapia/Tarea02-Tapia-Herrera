...
int main(int argc, char *argv[]) {
	...
	uint16_t echoServPort;  	//puerto del servidor
	std::string ipServidor;	//la ip del servidor
	std::string root_dir;	//la ruta al contenido de las páginas
	std::string notFoundFile;

	//echoServPort  = argumentos->getArgs().PORT;

	std::ifstream jsonFile("../bin/ajuste.json");

	jsonFile.is_open();
	json ajusteJson;
	jsonFile >> ajusteJson;

	ipServidor = ajusteJson.at("ip");
	echoServPort = ajusteJson.at("puerto");
	root_dir = ajusteJson.at("root_dir");
	notFoundFile = ajusteJson.at("notFoundFile");



	try {
		TCPServerSocket servSock(ipServidor, echoServPort);     // Server Socket object
		for (;;) {   // Run forever
			HandleTCPClient(servSock.accept(), root_dir, notFoundFile);       // Wait for a client to connect
		}
	} catch (SocketException &e) {
